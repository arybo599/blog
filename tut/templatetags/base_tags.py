...
@register.simple_tag
def title(data="وب سایت آموزشی"):
    return data


@register.inclusion_tag("tut/partials/category_navbar.html")
def category_navbar():
    return {
         "category": Category.objects.filter(status=True)
    }

@register.inclusion_tag("tut/partials/sidebar.html")
...
def hot_articles():
    last_month = datetime.today() - timedelta(days=30)
    content_type_id = user_type = ContentType.objects.get(app_label='tut', model='article').id
    return {
         "articles": Article.objects.published().annotate(
