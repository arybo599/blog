...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            email = EmailMessage(
                "دیدگاه جدید", 
                "دیدگاه جدیدی برای مقاله ای  '{}' که شما نویسنده آن هستید ارسال شده:\n{}{}".format(article,current_site,reverse('tut:details', kwargs={'slug': article.slug})),
                to=[author_email]
                )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                "پاسخ به دیدگاه شما", 
                "ثبت شده است.برای مشاهده بر روی لینک زیر کلیک کنید:  '{}'پاسخی به دیدگاه شما در مقاله:\n{}{}".format(article,current_site,reverse('tut:details', kwargs={'slug': article.slug})),
                to=[parent_email]
                )
            email.send()
